...
export task=cola
export input_dir=../glue_datasets
export outfolder=output-${task}-dict
...
python -u finetune.py \
  ...
  --task_name $task\
  --train_file ${input_dir}/${task}/train.prc.json \
  --validation_file ${input_dir}/${task}/validation.prc.json \
  --test_file ${input_dir}/${task}/test.prc.json \
  --dict_file ${input_dir}/${task}/vocab.90.json \
  --do_train \
  --do_eval \
  --do_predict \
